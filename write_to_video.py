...
while True:
    ...
    if writer is None:
        ...
        #break the image into its RGB components, then construct the RGB representation of each fram individually
        (B, G, R) = cv2.split(frame)
        R = cv2.merge([zeros, zeros, R])
        G = cv2.merge([zeros, G, zeros])
        B = cv2.merge([B, zeros, zeros])

        #construct the final output frame, sotring the original frame at the top-left, the red channel at the top right,
        ...
        #write the output file to frame
        writer.write(output)
